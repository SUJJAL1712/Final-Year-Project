...
if index_prices:
    ...
    fig = viz.plot_normalized_comparison(display_prices, events_filtered)
...
with col1:
    st.metric("Total Events", len(events_filtered))
    if "event_type" in events_filtered.columns:
...
if not events_filtered.empty:
    display_cols = ["date", "event", "category", "severity", "event_type"]
