...
def _make_realistic_series(n: int = 500, seed: int = 42) -> pd.Series:
    """
    Create a realistic-looking price series with controlled randomness.

    Uses a fixed seed so tests are deterministic and reproducible.
    """
...
class TestDirectionalChangeDetector:
        ...
        if len(summaries) >= 2:
            ...
            for i in range(1, len(directions)):
                assert directions[i] != directions[i - 1], (
                    "Consecutive DCs should alternate"
                )
    ...
    def test_empty_series(self):
        ...
        assert detector.detect(
            pd.Series([100], index=pd.bdate_range("2020-01-01", periods=1))
        ) == []
...
class TestMultiThresholdDC:
    def test_scaling_law_data(self):
        ...
        prices = _make_realistic_series(500, seed=42)
        ...
        if len(scaling) >= 2:
            assert (
                scaling.iloc[0]["num_dc_events"]
                >= scaling.iloc[-1]["num_dc_events"]
            )
...
class TestDCFeatureExtractor:
    def test_feature_extraction(self):
        ...
        prices = _make_realistic_series(200, seed=42)
